...
import { postSchemaValidator } from "@/lib/validators";
...
export function Editor({ post }: EditorProps) {
  ...
  const initializeEditor = React.useCallback(async () => {
    ...
    const body = postSchemaValidator.parse(post);
      ...
      <div className="grid w-full gap-10">
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center space-x-10">
            ...
            <p className="text-muted-foreground text-sm">{post.published ? "Published" : "Draft"}</p>
          </div>
        ...
        <div className="prose prose-stone dark:prose-invert mx-auto w-[800px]">
          ...
          <p className="text-sm text-gray-500">
            Use <kbd className="bg-muted rounded-md border px-1 text-xs uppercase">Tab</kbd> to open the
            command menu.
          </p>
        </div>
      </div>
